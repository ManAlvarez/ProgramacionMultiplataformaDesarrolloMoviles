...
public class PedidosTramiteCienteActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_pedidos_tramite_ciente);

        idKey = getIntent().getStringExtra("idKey");
        nombre = getIntent().getStringExtra("nombre");
        apellidos = getIntent().getStringExtra("apellidos");

        RecyclerView recyclerViewPedidos = (RecyclerView) findViewById(R.id.recyclerView);
        recyclerViewPedidos.setLayoutManager(new LinearLayoutManager(this));
        consultarListaPedidosCliente();
        Adaptador adaptador = new Adaptador(listaPedidos);
        recyclerViewPedidos.setAdapter(adaptador);
    }

    /**
     * Método para consultar los pedidos en tramite del cliente.
     */

    public void consultarListaPedidosCliente() {

        PedidosObject pedido = null;
        int idKeyInt = Integer.parseInt(idKey);
        BD.abrir();
        Cursor cursor = BD.consultaPedidosClienteTramite(idKeyInt);
        cursor.moveToFirst();
